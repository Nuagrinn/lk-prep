...
## Value semantics: что копируется, а что шарится

В Go очень важно держать в голове одну базовую вещь: присваивание, передача аргумента в функцию, возврат значения из функции и переменная `range` работают через значение. То есть Go берет значение и копирует его. Но дальше начинается самое интересное: разные значения устроены по-разному. У массива значение - это сами элементы. У слайса значение - это маленький header, внутри которого лежит ссылка на массив. Поэтому фраза "Go копирует значение" верна и для массива, и для слайса, но последствия разные.

Если совсем коротко:

| Конструкция | Что копируется | Что остается общим | Что видно снаружи |
|---|---|---|---|
| `[N]T` | весь массив целиком | ничего | изменение элемента в копии не видно |
| `*[N]T` | значение указателя | массив, на который он указывает | изменение элемента видно |
| `[]T` | slice header: pointer, len, cap | underlying array | изменение элемента видно, изменение len/cap не видно |
| `append(s, x)` без возврата | новый локальный slice header | иногда тот же array, иногда новый | внешний `len` не меняется |
| `s[i:j]` | новый slice header | тот же underlying array | изменение общих элементов видно |
| `s[i:j:k]` | новый slice header с ограниченным `cap` | тот же underlying array до append | append чаще уходит в новый array |
| `for _, v := range s` | элемент копируется в `v` | original element не становится `v` | изменение `v` не меняет элемент |

Это не частная особенность слайсов, а общий стиль языка. В Go нет скрытой автоматической передачи "по ссылке" для обычных параметров функции. Если функция принимает `func f(x T)`, она получает свое значение `x`. Внутри `f` можно менять эту локальную переменную сколько угодно, но сама переменная вызывающего кода от этого не станет другой. Чтобы функция могла поменять объект вызывающего кода напрямую, ей нужно дать значение, через которое можно добраться до того же объекта: например указатель, слайс с общим underlying array, map, channel, interface со ссылочным содержимым и так далее.

### Что значит "копируется значение"
...
Go ведет себя так, будто создает новое значение того же типа и кладет туда содержимое старого значения. Для простого `int` это очевидно:
...
`b` - это другая переменная. Она получила копию числа `10`. После этого `a` и `b` живут независимо.

С массивом идея такая же, просто значение больше:
...
Массив в Go - полноценное value type значение. Его элементы являются частью самого значения массива. Поэтому при присваивании `[3]int` копируются все три элемента. Если это `[1000000]int`, семантически копируется весь массив. Компилятор и runtime могут оптимизировать конкретный машинный код, но смысл программы именно такой: после копирования это два независимых массива.

То же самое при передаче в функцию:
...
func modify(points [2]Point) {
	points[0].X = 100
}

func main() {
	points := [2]Point{{X: 1}, {X: 2}}
	modify(points)
	fmt.Println(points[0].X) // 1
}
```

Функция получила копию массива. Даже если элементы массива - структуры, копируются и эти структуры тоже. `points[0].X = 100` меняет поле внутри локальной копии массива, а не исходный массив в `main`.

Если хотим менять исходный массив, надо передать указатель:
...
func main() {
	points := [2]Point{{X: 1}, {X: 2}}
	modify(&points)
	fmt.Println(points[0].X) // 100
}
```

Теперь копируется не массив, а значение указателя. Сам указатель тоже передан по значению: внутри функции есть копия адреса. Но обе копии адреса указывают на один и тот же массив. Поэтому изменение элемента через указатель видно снаружи.

### Почему со слайсом иначе

Слайс часто называют "ссылочным типом", но это опасное упрощение. Точнее так: слайс сам передается по значению, но его значение содержит ссылку на массив.
...
не копируются элементы `1, 2, 3`. Копируется только header:

```text
s header: ptr -> array[0], len=3, cap=3
t header: ptr -> array[0], len=3, cap=3

underlying array: [1, 2, 3]
```

После `t := s` у нас два разных header-а, но они смотрят на один underlying array. Поэтому:
...
Здесь не произошло "изменение слайса `s`" как переменной. Изменился элемент массива, на который смотрят оба слайса. `s` и `t` - разные slice header-ы, но `ptr` внутри них указывает на одно и то же место.

Именно здесь рождается главная путаница. Если менять элемент через слайс, изменение видно всем, кто смотрит на тот же массив. Если менять сам header - `len`, `cap`, `ptr` - это изменение касается только конкретной копии header-а.

### Слайс как аргумент функции

Когда слайс передается в функцию, копируется header:
...
```text
main x header:   ptr -> array[0], len=3, cap=3
touch values:    ptr -> array[0], len=3, cap=3

same array:      [1, 2, 3]
```

`values[0] = 100` идет по `ptr` в общий массив. Поэтому `x` потом видит `[100 2 3]`.

Но теперь другой пример:

```go
func addOne(values []int) {
...
`append` возвращает новый slice header. Он может указывать на тот же array, а может на новый. Но в любом случае результат присваивается локальной переменной `values`. Внешняя переменная `x` остается старым header-ом со старым `len`.

Схематично:

```text
before:
main x:        ptr -> old array, len=3, cap=3
add values:    ptr -> old array, len=3, cap=3

after append inside addOne:
main x:        ptr -> old array, len=3, cap=3
add values:    ptr -> new array, len=4, cap>=4
```

Даже если capacity хватило и новый массив не выделился, внешний `len` все равно не изменится:
...
func main() {
	...
	addOne(x)
	fmt.Println(x, len(x), cap(x)) // [1 2] 2 4
}
```

Внутри `addOne` запись могла попасть в тот же underlying array на позицию `array[2]`, но `x.len` снаружи остался `2`. Поэтому `fmt.Println(x)` показывает только первые два элемента. Данные "за len" могут физически лежать в массиве, но через этот slice header они не видны.

Правильный контракт для функции, которая добавляет элементы:

```go
func addOne(values []int) []int {
	return append(values, 4)
}
...
На ревью это один из первых вопросов: если функция append-ит в переданный слайс, возвращает ли она новый слайс вызывающему коду?

### Что происходит при срезе `s[i:j]`

Срез не копирует элементы. Он создает новый slice header, который смотрит в тот же underlying array, но с другим `ptr`, `len` и `cap`.
...
```go
s[0] = 200
fmt.Println(a) // [10 200 30 40 50]
```

Мы поменяли не "копию среза", а элемент общего массива.

Capacity у обычного `s[i:j]` считается от `i` до конца capacity исходного слайса или массива. Поэтому `append` к subslice может перезаписать элементы, которые логически принадлежат другому участку исходного слайса.

```go
s := []int{1, 2, 3, 4}
t := s[1:3] // [2 3], cap обычно 3: элементы 2,3,4
...
`append` в `t` использовал свободную capacity внутри того же underlying array и записал `99` поверх бывшего `s[3]`.

### Что меняет full slice expression `s[i:j:k]`

Full slice expression задает не только начало и конец видимой части, но и конец capacity:
...
Почему `s` не испортился? Потому что у `t` `len == cap`. Для `append` нет свободного места в разрешенном capacity-окне, поэтому runtime вынужден выделить новый array, скопировать туда `[2 3]`, добавить `99` и вернуть новый header. После этого `t` смотрит уже не на исходный массив.

Важно: `s[1:3:3]` не копирует элементы сразу. До `append` `t` все еще смотрит в исходный array. Full slice expression ограничивает именно capacity, то есть управляет тем, сможет ли будущий `append` писать дальше в тот же array.

### Почему `range` не дает ссылку на элемент
...
переменная `item` - это новая переменная цикла, в которую на каждой итерации копируется очередной элемент. Если `items` имеет тип `[]Task`, то `item` имеет тип `Task`, а не `*Task`.
...
fmt.Println(items[0].Done, items[1].Done) // false false
```

Мы меняли поле у копии структуры. Исходный элемент слайса не изменился.

Правильно менять по индексу:
...
fmt.Println(items[0].Done, items[1].Done) // true true
```

Здесь `items[i]` обращается к настоящему элементу underlying array, а не к копии в переменной цикла.

Если слайс хранит указатели, картина другая:
...
Теперь `item` - копия указателя. Сам указатель копируется по значению, но обе копии указывают на тот же `Task`. Поэтому изменение `item.Done` видно. Это та же логика, что и с `*[N]T`: копируется адрес, но объект по адресу общий.

### Главная формула для собеседования

Когда видишь присваивание, параметр функции, `range` или `append`, задай себе четыре вопроса:

1. Какой тип значения копируется?
2. Внутри этого значения лежат сами данные или ссылка на данные?
3. Я меняю само скопированное значение или объект, на который оно указывает?
4. Если был `append`, вернул ли я новый slice header туда, где он нужен?

Для массивов ответ обычно простой: массив копируется целиком, изменения в копии не видны.

Для слайсов ответ двухслойный: header копируется, array может быть общим. Изменения элементов видны через общий array, но изменения header-а не видны без возврата нового слайса.
